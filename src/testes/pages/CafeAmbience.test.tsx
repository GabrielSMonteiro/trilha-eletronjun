...
describe('CafeAmbience Page', () => {
  ...
  it('alterna o play/pause global', async () => {
    ...
    // O botão de play/pause global tem um ícone de Play inicialmente
    const buttons = screen.getAllByRole('button');
    // Procuramos pelo botão que tem as classes do toggle global
    const toggleBtn = buttons.find(btn => btn.className.includes('h-14 w-14 rounded-full'));
    
    if (toggleBtn) {
      fireEvent.click(toggleBtn);
      // Depois de clicar, fica com ícone de Pause, e depois clicar de novo para voltar a Play
      fireEvent.click(toggleBtn);
    }
  });
  ...
  it('altera o volume master', async () => {
    ...
    await waitFor(() => {
      // Radix slider uses keyboard navigation
