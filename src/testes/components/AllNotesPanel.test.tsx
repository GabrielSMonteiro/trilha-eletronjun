...
describe('AllNotesPanel', () => {
  ...
  it('chama onClose ao clicar no botão fechar', async () => {
    ...
    // O botão de fechar é o primeiro botão (sem texto, apenas ícone X)
  ...
  it('carrega e exibe a lista de notas', async () => {
    ...
    await waitFor(() => {
      // Como o mock single retorna Introdução ao React, todos os itens terão esse título no teste
  ...
  it('expande a nota ao clicar', async () => {
    ...
    // Apenas testando que não quebra, pois o texto curto já aparece na visualização padrão e a lógica de expandir
    // apenas remove o limite de 100 caracteres.
    expect(screen.getByText('Anotação mais longa')).toBeInTheDocument();
  });
});
