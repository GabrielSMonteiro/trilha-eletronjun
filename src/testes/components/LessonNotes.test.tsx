...
describe('LessonNotes', () => {
  ...
  it('modo floating renderiza botão quando isMobile=false', async () => {
    // Sobrescrever window.innerWidth para simular desktop
    global.innerWidth = 1024;
    global.dispatchEvent(new Event('resize'));
    
    render(<LessonNotes {...defaultProps} embedded={false} />);
    
    // Na inicialização ele só mostra o botão flutuante com sr-only / title
    await waitFor(() => {
      // Como o ícone é renderizado ou o aria-label, podemos checar o aria-label
      expect(screen.getByLabelText('Anotações')).toBeInTheDocument();
    });
    
    // Clicar no botão para abrir o painel flutuante
