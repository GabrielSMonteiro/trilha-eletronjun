...
// ── Supabase mock ─────────────────────────────────────────────────────────────
// NOTE: All mock variables referenced inside vi.mock factory MUST use
...
describe('AuthBackgroundCarousel', () => {
  ...
  it('uses "Background" alt text for images with null title', async () => {
    ...
    const imgs = screen.getAllByRole('img');
    expect(imgs[2]).toHaveAttribute('alt', 'Background'); // title is null
  });
  ...
  it('wraps back to first image after cycling through all', async () => {
    ...
    await act(async () => {
      vi.advanceTimersByTime(15000); // 3 × 5 seconds
    });
