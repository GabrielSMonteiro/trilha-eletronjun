...
const { mockFrom, mockInsert, mockUpdate, mockDelete } = vi.hoisted(() => {
  ...
  const mockFrom = vi.fn((table: string) => {
    switch (table) {
      case 'categories':
        // If single is called (checking existence), let's return null to mean "does not exist"
        return createChain([
...
describe('AdminCategories Component', () => {
  ...
  it('renders and loads categories', async () => {
    ...
    await waitFor(() => {
      ...
      // Category 1 should have 1 lesson based on the mock
  ...
  it('deletes a category when confirmed', async () => {
    ...
    // Categoria 2 has 0 lessons, so it can be deleted
    // Delete buttons are the second action button on each card
    const deleteButtons = screen.getAllByTestId('icon-Trash2');
    await user.click(deleteButtons[1].closest('button')!); // Categoria 2 is the second one
  ...
  it('prevents deleting a category with lessons', async () => {
    ...
    // Categoria 1 has 1 lesson, should show error toast
    const deleteButtons = screen.getAllByTestId('icon-Trash2');
    await user.click(deleteButtons[0].closest('button')!);

    // Shows alert dialog WITH the warning text, and disabled button
    expect(screen.getByText(/não pode ser excluída/i)).toBeInTheDocument();
    
    const confirmBtn = screen.getByRole('button', { name: 'Excluir' });
    expect(confirmBtn).toBeDisabled();
    
    // Also, the original onClick handler sets the toast
    // Wait, the component changed in the latest code? 
    // `deleteCategory.lessons_count > 0` shows a toast if trying to bypass, 
    // but now it just disables the button in the UI. We verify the button is disabled.
  });
});
