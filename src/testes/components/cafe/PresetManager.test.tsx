...
describe('PresetManager', () => {
  ...
  it('exibe erro caso serviço de salvar falhe', async () => {
    const user = userEvent.setup();
    (cafeService.savePreset as any).mockResolvedValue(null); // Falha
  ...
  it('permite deletar um preset do usuário', async () => {
    ...
    const deleteBtn = item?.querySelectorAll('button')[1]; // Segundo botão é o delete
    if (deleteBtn) {
  ...
  it('permite carregar um preset clicando nele', async () => {
    ...
    const item = screen.getByText('Foco Total').closest('.flex');
    const downloadBtn = item?.querySelectorAll('button')[0]; // Primeiro botão é o download
    if (downloadBtn) {
